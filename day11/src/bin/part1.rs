...
fn main()
{
    ...
    let start = Instant::now();
    let mut sum_pair_lengths = 0u32;
    for i in 0..galaxies.len()-1
    {
        for j in i+1..galaxies.len()
        {
            let diff = galaxies[j] - galaxies[i];
            sum_pair_lengths += (diff.x + diff.y.abs()) as u32;
        }
    }
...
fn parse_galaxies(data: &str) -> Vec<Vec2>
{
        ...
        if let Some(c) = iter.peek()
        {
            match c
            {
                ...
                '\r' => {
                    ...
                    rows += 1 + if num_galaxies_in_row == 0 { 1 } else { 0 };
    ...
    // Expand columns
    {
        ...
        for (_, galaxy) in galaxies.iter_mut().enumerate()
        {
            ...
            {
                offset += galaxy.x - current_column - 1;
